...
# CRA models may not be present depending on plaid-python version.
try:
    ...
    ConsumerReportUserIdentity = None
    ConsumerReportPermissiblePurpose = None
    CraCheckReportBaseReportGetRequest = None
    CraCheckReportPDFGetRequest = None
    CraCheckReportIncomeInsightsGetRequest = None
    CraCheckReportPartnerInsightsGetRequest = None
    CraPDFAddOns = None
    CRA_AVAILABLE = False
try:
    from plaid.model.client_user_identity import ClientUserIdentity
except Exception:
    ClientUserIdentity = None
...
try:
    ...
    LinkTokenCreateRequestStatements = None
...
try:
    ...
    CRA_OPTIONS_AVAILABLE = True
except Exception:
    LinkTokenCreateRequestCraOptions = None
    CRA_OPTIONS_AVAILABLE = False
...
def create_link_token():
    ...
    try:
        ...
        if STATEMENTS_AVAILABLE and Products('statements') in products:
        ...
        if is_cra and CRA_AVAILABLE:
            # Use user_token if available, otherwise use user_id
            if user_token:
                request['user_token'] = user_token
            elif user_id:
                request['user_id'] = user_id
            if ConsumerReportPermissiblePurpose and CRA_OPTIONS_AVAILABLE:
...
def create_user_token():
    ...
    try:
        ...
        cra_products = ["cra_base_report", "cra_income_insights", "cra_partner_insights"]
        if CRA_AVAILABLE and ClientUserIdentity and any(product in cra_products for product in PLAID_PRODUCTS):
        ...
        if (CRA_AVAILABLE and error_body.get('error_code') == 'INVALID_FIELD' and
...
def statements():
    if not STATEMENTS_AVAILABLE:
        return jsonify({'error': {'error_type': 'STATEMENTS_DISABLED', 'error_message': 'Statements models unavailable in installed plaid-python'}}), 400
    try:
...
def cra_check_report():
    if not CRA_AVAILABLE:
        return jsonify({'error': {'error_type': 'CRA_DISABLED', 'error_message': 'CRA models unavailable in installed plaid-python'}}), 400
    try:
        # Use user_token if available, otherwise use user_id
        if user_token:
